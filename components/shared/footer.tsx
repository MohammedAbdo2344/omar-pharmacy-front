...
export default function Footer() {
  ...
  const workingHours = config?.working_hours || t('hours');
  const instagramUrl = config?.instagram || 'https://instagram.com';
  const facebookUrl = config?.facebook || 'https://facebook.com';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="text-blue-100/70 text-sm">{address}</li>
              <li className="text-blue-100/70 text-sm">
                <a href={`tel:${phone}`} className="hover:text-white transition-colors">
                  {phone}
                </a>
              </li>
              <li className="text-blue-100/70 text-sm">{workingHours}</li>
            </ul>
            <a
              href={`tel:${phone}`}
